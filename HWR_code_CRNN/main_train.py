...
class HW_Recognizer():
    ...
    def train(self):
        ...
        ## inference
        cnn_ouput,seq_lens_c = inference.cnn_net(images,seq_lens,self.IS_TRAINING)
        logistics = inference.bi_lstmm_net(cnn_ouput,seq_lens_c,self.IS_TRAINING)
        # md_lstm_output = inference.mdlstm_network(cnn_ouput)
        # logistics = inference.fc_network(lstm_output)
        # ctc_loss
        cost = self.compute_cost(labels_sparse=labels_sparse, inputs=logistics, sequence_length=seq_lens_c)
        # 正则化
        ...
        # 通过beamSearch获取预测结果
        pred_sparse = self.get_prediction(inputs=logistics, sequence_length=seq_lens_c)
        ...
        with tf.Session(config=config) as sess:
            ...
            try:
                ...
                while not coord.should_stop():
                    # start = time.time()
                    now_time_str = time.strftime('%m-%d_%H:%M:%S', time.localtime(time.time()))
                    ## 从TFrecord中获取到label的sequence(长度定长为最大长度，不足padding。是数字序列,数字是字符在字典中的位置。)
                    ## sequence转为sparse,然后通过feed_dict传入placeholder
                    print("New batch begin...times:%04d" % (loop_batch_times)," ,now time:",now_time_str)
                    ...
                    # labels_seq_,seq_lens_,labels_sparse_,merged_,labels_ = sess.run(
                    #                 [labels_seq,seq_lens_f, labels_sparse_f, merged,labels],
                    #                 feed_dict=feed_dict)
                    # print("labels_seq_=",labels_seq_," ,seq_lens_=",seq_lens_," ,labels_sparse_=",labels_sparse_," ,labels_=",labels_)
                    ...
                    if (global_steps_ % 1 == 0):
                        for i in range(0, labels_seq_.shape[0], 50):
