...
IMAGE_WIDTH =  100
CHARS_MAX_NUM = 2      ## 15, 单个label中最大字符数，在label存入tfrecord之前固定长度为15，不足padding
...
def _gen_tfrecord_from_data(path,total_data):
    ...
    path = path + "_%d" % (IMAGE_HEIGHT)
    path = path + "_%d" % (IMAGE_WIDTH)
    ...
    for i in range(len(total_data)):
        ...
        image_label = data[2:]
        ## label 填充到CHARS_MAX_NUM，padding 0
